...
def process_parquet_files_optimized(folder_path: str | Path, output_name: str = "statistics.json") -> None:
    ...
    for file_idx, file_path in enumerate(parquet_files):
        try:
            ...
            if 'observation.state' not in df.columns and 'action' not in df.columns:
            ...
            for _, row in df.iterrows():
                if 'observation.state' in row and isinstance(row['observation.state'], np.ndarray):
                    observation_batch.append(row['observation.state'])
                if 'action' in row and isinstance(row['action'], np.ndarray):
                    action_batch.append(row['action'])

            if not observation_batch and not action_batch:
                continue
    ...
    if obs_accumulator['count'] == 0 and act_accumulator['count'] == 0:
    ...
    if obs_accumulator['count'] > 0:
        ...
        output_dict.update({"observation.state": observation_stats})
    if act_accumulator['count'] > 0:
        action_stats = calculate_final_statistics(act_accumulator, "action")
        output_dict.update({"action": action_stats})
