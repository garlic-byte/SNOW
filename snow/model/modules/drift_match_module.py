...
# === Copy from {Generative Modeling via Drifting} ===
...
class DriftActionHead(nn.Module):
    """Action head component for draft matching diffusion policy."""
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> dict:
        ...
        batch_size, action_horizon, action_dim = pred_actions.shape
        pred_actions = pred_actions.view(-1, action_dim)
        actions = actions.view(-1, action_dim)
        action_mask = action_mask.view(-1, action_dim)
        with torch.no_grad():
            velocity = compute_drift(pred_actions, actions, self.config.drift_temperature)
            actions = (pred_actions + velocity).detach()

        # Slice out only the action portion of pred and target.
        action_loss = F.mse_loss(pred_actions, actions, reduction="none") * action_mask
